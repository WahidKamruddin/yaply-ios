...
final class ConversationRepository {
    ...
    func createDirectConversation(userId: UUID, otherUserId: UUID) async throws -> UUID {
        ...
        struct DirectConvInsert: Encodable {
            let is_group: Bool
            let created_by: String
        }
        let conv: NewConv = try await supabase
            .from("conversations")
            .insert(DirectConvInsert(is_group: false, created_by: userId.uuidString))
            .select("id")
            .single()
            .execute()
            .value
    ...
    func createGroupConversation(userId: UUID, memberIds: [UUID], name: String) async throws -> UUID {
        ...
        let conv: NewConv = try await supabase
            .from("conversations")
            .insert(GroupConvInsert(is_group: true, name: name, created_by: userId.uuidString))
            .select("id")
            .single()
            .execute()
            .value
